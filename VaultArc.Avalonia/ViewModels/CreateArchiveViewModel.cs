...
public partial class CreateArchiveViewModel(VaultArcFacade facade) : ViewModelBase
{
    ...
    {
        if (string.IsNullOrWhiteSpace(DestinationPath)) { StatusMessage = "Select a destination archive path first."; return; }
        var inputs = InputPaths.Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
        if (inputs.Count == 0) { StatusMessage = "Add at least one input file or folder before queueing."; DeleteDestinationPlaceholderIfEmpty(); return; }
        var existingInputs = inputs.Where(path => File.Exists(path) || Directory.Exists(path)).ToList();
        if (existingInputs.Count == 0) { StatusMessage = "None of the input paths were found on disk."; DeleteDestinationPlaceholderIfEmpty(); return; }
        var extension = Path.GetExtension(DestinationPath);
        if (extension.Equals(".7z", StringComparison.OrdinalIgnoreCase)) { StatusMessage = "7z creation is not supported. Use .zip, .tar, .tar.gz, .tar.xz, or .arc."; DeleteDestinationPlaceholderIfEmpty(); return; }
        if (extension.Equals(".rar", StringComparison.OrdinalIgnoreCase)) { StatusMessage = "RAR creation is not supported. Use .zip, .tar, .tar.gz, .tar.xz, or .arc."; DeleteDestinationPlaceholderIfEmpty(); return; }
        if (extension.Equals(".arc", StringComparison.OrdinalIgnoreCase) && string.IsNullOrWhiteSpace(Password)) { StatusMessage = "Encrypted .arc creation requires a password."; DeleteDestinationPlaceholderIfEmpty(); return; }
        var request = new ArchiveCreateRequest(DestinationPath, existingInputs, Preset, string.IsNullOrWhiteSpace(Password) ? null : Password, EncryptionProfile);
    ...
    private void DeleteDestinationPlaceholderIfEmpty()
    {
        ...
        try { if (File.Exists(DestinationPath)) { var info = new FileInfo(DestinationPath); if (info.Length == 0) File.Delete(DestinationPath); } } catch { }
    }
}
