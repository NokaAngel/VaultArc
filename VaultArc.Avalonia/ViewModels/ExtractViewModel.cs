...
public partial class ExtractViewModel(VaultArcFacade facade) : ViewModelBase
{
    ...
    public async Task<OperationResult> QueueExtractAsync()
    {
        ...
        if (openResult.IsFailure)
        {
            ...
            return OperationResult.Failure(openResult.Error?.Code ?? "archive.open_failed", openResult.Error?.Message ?? "Unable to access archive.", openResult.Error?.Exception);
        }
        var request = new ArchiveExtractRequest(ArchivePath, DestinationPath, OverwriteExisting, string.IsNullOrWhiteSpace(Password) ? null : Password);
