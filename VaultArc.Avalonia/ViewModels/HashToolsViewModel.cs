...
{
    [ObservableProperty] private string _leftFilePath = string.Empty;
    [ObservableProperty] private string _rightFilePath = string.Empty;
    [ObservableProperty] private VaultArcHashAlgorithm _algorithm = VaultArcHashAlgorithm.Sha256;
    [ObservableProperty] private string _comparisonResult = string.Empty;
    ...
    public async Task HashSingleAsync(string filePath)
    {
        ...
        if (result.IsFailure || result.Value is null) { StatusMessage = result.Error?.Message ?? "Hash failed."; return; }
    ...
    public async Task CompareAsync()
    {
        ...
        if (result.IsFailure || result.Value is null) { ComparisonResult = result.Error?.Message ?? "Hash compare failed."; return; }
        ComparisonResult = result.Value.Matches ? "Hashes match." : "Hashes do not match.";
    }
