...
public sealed class JobQueueViewModel : ViewModelBase
{
    ...
    private void RefreshCore()
    {
        ...
        {
            seen.Add(job.JobId);
            if (_jobLookup.TryGetValue(job.JobId, out var existing)) { existing.UpdateFrom(job); continue; }
        ...
        foreach (var staleId in staleIds) { if (!_jobLookup.Remove(staleId, out var staleItem)) continue; _ = Jobs.Remove(staleItem); }
        for (var targetIndex = 0; targetIndex < snapshot.Count; targetIndex++)
        {
            var targetJobId = snapshot[targetIndex].JobId;
            if (!_jobLookup.TryGetValue(targetJobId, out var targetItem)) continue;
            var currentIndex = Jobs.IndexOf(targetItem);
            if (currentIndex >= 0 && currentIndex != targetIndex) Jobs.Move(currentIndex, targetIndex);
        }
    ...
    {
        lock (_refreshGate) { _hasPendingRefresh = true; if (_isRefreshScheduled) return; _isRefreshScheduled = true; }
        if (_uiContext is not null) { _uiContext.Post(_ => ProcessRefreshQueue(), null); return; }
        ProcessRefreshQueue();
    }
    ...
    private void ProcessRefreshQueue()
    {
        while (true)
        {
            lock (_refreshGate) { _hasPendingRefresh = false; }
            RefreshCore();
            lock (_refreshGate) { if (_hasPendingRefresh) continue; _isRefreshScheduled = false; return; }
        }
    }
...
public sealed partial class JobQueueItemView : ObservableObject
{
    ...
    [ObservableProperty] private string _elapsedText = "Elapsed: 00:00";
    [ObservableProperty] private string _etaText = "ETA: --";

    public JobQueueItemView(Guid jobId) { JobId = jobId; }
    ...
    {
        Title = job.Title;
        StateLabel = job.State switch { JobState.Pending => "Pending", JobState.Running => "Running", JobState.Completed => "Completed", JobState.Failed => "Failed", JobState.Cancelled => "Cancelled", _ => job.State.ToString() };
