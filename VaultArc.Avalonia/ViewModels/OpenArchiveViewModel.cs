    ...
    IFileTypeClassificationService fileTypeClassificationService) : ViewModelBase
{
    ...
    public async Task<OperationResult<ArchiveSummary>> OpenAsync(string archivePath)
    {
        ...
        try
        {
            ...
            foreach (var item in result.Value.Items)
            {
                ...
                var typeLabel = item.IsDirectory ? "Folder"
    ...
    public async Task<OperationResult<ArchivePreviewResult>> PreviewSelectedAsync()
    {
        ...
        PreviewText = result.Value.MimeType.StartsWith("text/", StringComparison.OrdinalIgnoreCase)
            ...
            : $"Preview available for {result.Value.MimeType}, size {result.Value.Data.Length} bytes.";
    ...
    public void RefreshVisibleItems()
    {
        ...
        VisibleItems.Clear();
        foreach (var child in children) VisibleItems.Add(child);
    ...
    {
        if (archiveNavigationService.NavigateUp()) RefreshVisibleItems();
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(folderPath)) { NavigateToRoot(); return; }
    ...
    public async Task<OperationResult<ArchiveLaunchResult>> OpenSelectedAsync()
    {
        ...
        if (SelectedItem.IsDirectory) { NavigateIntoSelectedFolder(); return OperationResult<ArchiveLaunchResult>.Failure("archive.folder_navigation", "Navigated into folder."); }
        ...
        else StatusMessage = result.Error?.Message ?? "Failed to open selected entry.";
        return result;
    }
    ...
    public async Task<OperationResult<ArchiveLaunchResult>> RunSelectedAsync()
    {
        ...
        else StatusMessage = result.Error?.Message ?? "Failed to run selected entry.";
        return result;
    }
    ...
    public async Task<OperationResult<ArchiveEditSession>> CreateEditSessionForSelectedAsync()
    {
        ...
        StatusMessage = result.IsSuccess ? "Opened entry for external edit." : result.Error?.Message ?? "Failed to open entry for editing.";
        return result;
    }

    public async Task<bool> HasEntryChangedAsync(ArchiveEditSession session)
    {
        if (!File.Exists(session.TempFilePath)) return false;
    ...
    public async Task<OperationResult> SaveEditedSessionAsync(ArchiveEditSession session)
    {
        ...
        StatusMessage = result.IsSuccess ? "Saved changes back into archive." : result.Error?.Message ?? "Failed to save changes into archive.";
        return result;
    }
    ...
    public async Task<OperationResult> RenameSelectedAsync(string newName)
    {
        ...
        var targetPath = string.IsNullOrWhiteSpace(folder) ? newName : $"{folder}/{newName}".Replace('\\', '/');
        var request = new ArchiveOpenRequest(ArchivePath, EffectivePassword);
        var result = await facade.RenameEntryAsync(request, SelectedItem.FullPath, targetPath, CancellationToken.None);
        if (result.IsSuccess) { await OpenAsync(ArchivePath); StatusMessage = "Entry renamed."; }
        else StatusMessage = result.Error?.Message ?? "Rename failed.";
        return result;
    }
    ...
    public async Task<OperationResult> DeleteSelectedAsync()
    {
        ...
        if (result.IsSuccess) { await OpenAsync(ArchivePath); StatusMessage = "Entry deleted."; }
        else StatusMessage = result.Error?.Message ?? "Delete failed.";
        return result;
    }
    ...
    public async Task<OperationResult> QueueExtractSelectedAsync(IReadOnlyList<ArchiveItem> items)
    {
        ...
        { StatusMessage = "Set archive path and destination folder."; return OperationResult.Failure("extract.missing_paths", "Archive or destination path is empty."); }
        if (items.Count == 0) { StatusMessage = "Select entries to extract."; return OperationResult.Failure("extract.no_selection", "No entries selected."); }
        ...
        StatusMessage = result.IsSuccess ? $"Extraction queued ({items.Count} entries)." : result.Error?.Message ?? "Failed to queue extraction.";
        return result;
    }
    ...
    {
        if (SelectedItem is null) { CanOpenSelected = false; CanRunSelected = false; return; }
        if (SelectedItem.IsDirectory) { CanOpenSelected = true; CanRunSelected = false; return; }
    ...
    private void RefreshBreadcrumbs()
    {
        ...
        var segments = current.Split('/', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
    ...
    {
        if (item is null) { SelectedDisplayName = "-"; SelectedTypeLabel = "-"; SelectedSizeLabel = "-"; SelectedModifiedLabel = "-"; SelectedFullPath = "-"; return; }
